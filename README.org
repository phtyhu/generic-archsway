...
- Edit =~/confrepo/insdot.conf.yaml= and add one corresponding line in the =- link:= part:
#+begin_src yaml
- link:
    ...
    ...
...
After that, run =~/confrepo/insdot= ,
...
according to the =- links:= part in =~/confrepo/insdot.conf.yaml= .
#+end_quote
